...
from code_factory.pooling import GeM,AdaptiveConcatPool2d,AdaptiveConcatPool1d
from code_factory.seq_aug import *
import logging
...
def denoise_channel(ts, bandpass=[1,40], signal_freq=100, bound=0.000125):
    ...
    return ts
#print(denoise_channel(np.ones((3000,7))).shape)
...
class TrainDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        seq = np.load(f"/data/crest/sleep/arrays/{id_}_{epoch}.npy")#3000,7
        seq = seq[:,:2]

        if self.CFG.augmentation.crop_len>0:
            l = np.random.randint(self.CFG.augmentation.crop_len)
...
class gru_model(nn.Module):
    ...
    def forward(self, embeds):
        ...
        embeds = embeds if self.do_embed ==False else self.layer1(embeds.permute(0, 2, 1)).permute(0, 2, 1)
...
class CNN_1d(nn.Module):
    def __init__(self, num_classes=400,dim=16,input_ch=128,verbose=False,pool="concat"):
        ...
        if pool in ('avg','concat','gem','max'):
            self.pool = SEQ_POOLING[pool]
            if pool == 'concat': in_size *= 2
        else:
            self.pool = None
        self.head = nn.Sequential(nn.Linear(in_size, dim),nn.ReLU(inplace=True),nn.Dropout(0.8),
...
class Classifier(nn.Module):
    def __init__(self, inch=12, kernel_size=3,num_classes=2,pool="avg"):
        ...
        self.head = nn.Sequential(nn.Linear(128, dim),nn.ReLU(inplace=True),nn.Dropout(0.5),
                nn.Linear(dim, num_classes))
...
kernel_dict={"tf_efficientnet_b0_ns":32,"tf_efficientnet_b2_ns":32,"tf_efficientnetv2_s":24,"tf_efficientnetv2_m":24,"tf_efficientnet_b5_ns":48,"tf_efficientnetv2_l":32
             ,"convnext_nano":80,"convnext_tiny":96,"convnext_small":96}
...
class stft_conv_more(nn.Module):
    def __init__(self,CFG):
        ...
        self.qishen_conv= nn.Sequential(
            ...
            nn.BatchNorm2d(12),nn.Mish(),
            nn.Conv2d(12, 24, 3, stride=1, padding=1, bias=False),
            nn.BatchNorm2d(24),nn.Mish(),
            )
...
def main(CFG : DictConfig) -> None:
    ...
    folds = pd.read_csv("/home/abebe9849/sleep/data/folds_0.csv")
    tmp = folds[folds["id"]=="5edb9d9"]
    tmp = tmp[tmp["epoch"]==1101]
    folds = folds.drop(index=tmp.index).reset_index(drop=True)
    test = pd.read_csv("/home/abebe9849/sleep/data/test_df0.csv")
    ...
    submission = pd.read_csv("/home/abebe9849/sleep/data/sample_sub_nishika.csv")
